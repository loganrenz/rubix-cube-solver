<template>
    <div
        class="solution-display bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 h-full overflow-hidden flex flex-col">
        ...
        <div v-else class="flex-1 overflow-y-auto no-scrollbar">
            ...
            <div v-for="(group, stage) in groupedSolution" :key="stage" class="mb-4">
                <h4
                    class="font-medium text-gray-700 dark:text-gray-300 mb-2 text-sm sticky top-0 bg-white dark:bg-gray-800 py-1">
                    {{ stage }}
                </h4>
                <div class="space-y-1">
                    <div v-for="(step, index) in group" :key="`${stage}-${index}`" :class="[
                        ...
                        <span v-if="step.description"
                            class="text-xs text-gray-500 dark:text-gray-400 ml-2 truncate max-w-[150px]">
                            {{ step.description }}
                        </span>
                    </div>
                </div>
        ...
        <details class="mt-4 text-sm">
            ...
            <div class="mt-2 p-3 bg-gray-100 dark:bg-gray-700 rounded text-xs space-y-1">
                <div><span class="font-mono font-medium">F/B/U/D/L/R</span> - Face turns (Front/Back/Up/Down/Left/Right)
                </div>
                <div><span class="font-mono font-medium">'</span> - Counter-clockwise (e.g., F' = Front
