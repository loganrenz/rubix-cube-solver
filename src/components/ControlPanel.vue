    ...
    <div class="control-panel p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-2 gap-3">
                <button @click="scramble" :disabled="isScrambling || animationState.isAnimating" class="btn-primary">
                    <span v-if="isScrambling">Scrambling...</span>
                    <span v-else>Scramble</span>
                </button>
                <button @click="solve" :disabled="isSolving || animationState.isAnimating || isSolved"
                    class="btn-primary">
        ...
        <div v-if="solution.length > 0" class="mb-6">
            ...
            <div class="mb-3">
                ...
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div class="bg-blue-600 h-2 rounded-full transition-all duration-300"
                        :style="{ width: `${(animationState.currentStep / solution.length) * 100}%` }"></div>
                </div>
            </div>

            <div class="grid grid-cols-4 gap-2 mb-3">
                <button @click="stepBackward" :disabled="animationState.currentStep === 0 || animationState.isAnimating"
                    class="btn-secondary text-sm px-2 py-1" aria-label="Step backward">
                    ⏮
                </button>
                <button v-if="!animationState.isAnimating || animationState.isPaused" @click="play"
                    :disabled="animationState.currentStep >= solution.length" class="btn-primary text-sm px-2 py-1"
                    aria-label="Play">
                    ▶
                </button>
                <button v-else @click="pause" class="btn-primary text-sm px-2 py-1" aria-label="Pause">
                    ⏸
                </button>
                <button @click="stepForward"
                    :disabled="animationState.currentStep >= solution.length || animationState.isAnimating"
                    class="btn-secondary text-sm px-2 py-1" aria-label="Step forward">
                    ⏭
                </button>
            ...
            <!-- Current stage info -->
            <div v-if="currentMove?.stage"
                class="text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded p-2">
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-3 gap-2 text-sm">
                <button v-for="move in basicMoves" :key="move" @click="applyMove(move)"
                    :disabled="animationState.isAnimating" class="btn-secondary py-1 px-2 text-xs font-mono">
                    {{ move }}
                </button>
            </div>
        </div>
        ...
        <div v-if="isSolved"
            class="mb-4 p-3 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-lg text-sm font-medium">
            🎉 Cube Solved!
        </div>
    </div>
</template>
