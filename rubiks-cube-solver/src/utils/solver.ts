import { Move, Color, SolverResult, SolutionStep } from '../types/cube'
...
export class LayerByLayerSolver {
  ...
  private isWhiteEdgeCorrect(targetColor: Color): boolean {
    ...
    switch (targetColor) {
      case Color.RED:
        return state.up.colors[1][2] === Color.WHITE && 
               state.right.colors[0][1] === Color.RED
      case Color.BLUE:
        return state.up.colors[0][1] === Color.WHITE && 
               state.back.colors[0][1] === Color.BLUE
      case Color.ORANGE:
        return state.up.colors[1][0] === Color.WHITE && 
               state.left.colors[0][1] === Color.ORANGE
      case Color.GREEN:
        return state.up.colors[2][1] === Color.WHITE && 
  ...
  private findWhiteEdge(targetColor: Color): { face: string; position: string } | null {
    ...
    const edges = [
      ...
      // Middle layer edges
      { face: 'F', position: 'right', colors: [state.front.colors[1][2], state.right.colors[1][0]] },
    ...
    for (const edge of edges) {
      if ((edge.colors[0] === Color.WHITE && edge.colors[1] === targetColor) ||
  ...
  private isEdgeAlignedForInsertion(targetColor: Color): boolean {
    ...
    switch (targetColor) {
      case Color.RED:
        return state.down.colors[1][2] === Color.WHITE || 
               state.right.colors[2][1] === Color.WHITE
      case Color.BLUE:
        return state.down.colors[2][1] === Color.WHITE || 
               state.back.colors[2][1] === Color.WHITE
      case Color.ORANGE:
        return state.down.colors[1][0] === Color.WHITE || 
               state.left.colors[2][1] === Color.WHITE
      case Color.GREEN:
        return state.down.colors[0][1] === Color.WHITE || 
               state.front.colors[2][1] === Color.WHITE
      default:
        return false
    }
  }
  ...
  private solveWhiteCorner(colors: Color[], description: string): void {
    ...
    while (!this.isWhiteCornerCorrect(colors) && attempts < maxAttempts) {
      ...
      if (state.front.colors[0][2] === Color.WHITE) {
        ...
        this.addMoves([Move.R, Move.U2, Move.R_PRIME, Move.U_PRIME, Move.R, Move.U, Move.R_PRIME], description, stage)
      }
  ...
  private isWhiteCornerCorrect(colors: Color[]): boolean {
    ...
    const corners = [
      { pos: [state.up.colors[2][2], state.front.colors[0][2], state.right.colors[0][0]], colors: [Color.WHITE, Color.GREEN, Color.RED] },
      { pos: [state.up.colors[0][2], state.right.colors[0][2], state.back.colors[0][0]], colors: [Color.WHITE, Color.RED, Color.BLUE] },
      { pos: [state.up.colors[0][0], state.back.colors[0][2], state.left.colors[0][0]], colors: [Color.WHITE, Color.BLUE, Color.ORANGE] },
      { pos: [state.up.colors[2][0], state.left.colors[0][2], state.front.colors[0][0]], colors: [Color.WHITE, Color.ORANGE, Color.GREEN] }
    ]
    
    for (const corner of corners) {
      if (this.cornersMatch(corner.pos, corner.colors) && 
  ...
  private findWhiteCorner(targetColors: Color[]): { layer: string; position: number } | null {
    ...
    for (let i = 0; i < topCorners.length; i++) {
      if (this.cornerContainsColors(topCorners[i], [Color.WHITE, ...targetColors]) &&
  ...
  private cornersMatch(corner1: Color[], corner2: Color[]): boolean {
    return corner1.every(color => corner2.includes(color)) &&
      ...
      if (edgePosition.layer === 'top') {
        ...
        if (state.front.colors[0][1] === colors[0] || state.front.colors[0][1] === colors[1]) {
          ...
          if (this.shouldGoRight(colors)) {
            this.addMoves([Move.U, Move.R, Move.U_PRIME, Move.R_PRIME, Move.U_PRIME, Move.F_PRIME, Move.U, Move.F], description, stage)
          } else {
            this.addMoves([Move.U_PRIME, Move.L_PRIME, Move.U, Move.L, Move.U, Move.F, Move.U_PRIME, Move.F_PRIME], description, stage)
          }
        }
      } else {
  ...
  private isMiddleEdgeCorrect(colors: Color[]): boolean {
    ...
    const edges = [
      { pos: [state.front.colors[1][2], state.right.colors[1][0]], colors: [Color.GREEN, Color.RED] },
      { pos: [state.right.colors[1][2], state.back.colors[1][0]], colors: [Color.RED, Color.BLUE] },
      { pos: [state.back.colors[1][2], state.left.colors[1][0]], colors: [Color.BLUE, Color.ORANGE] },
      { pos: [state.left.colors[1][2], state.front.colors[1][0]], colors: [Color.ORANGE, Color.GREEN] }
    ]
    
    for (const edge of edges) {
      if (this.edgesMatch(edge.pos, edge.colors) && 
          this.edgesMatch(colors, edge.colors)) {
        return true
      }
    }
  ...
  private findMiddleLayerEdge(colors: Color[]): { layer: string; position: number } | null {
    ...
    for (let i = 0; i < topEdges.length; i++) {
      if (this.edgeContainsColors(topEdges[i], colors) && 
          !topEdges[i].includes(Color.YELLOW)) {
        return { layer: 'top', position: i }
      }
    }
    ...
    for (let i = 0; i < middleEdges.length; i++) {
      if (this.edgeContainsColors(middleEdges[i], colors) &&
          !this.isMiddleEdgeCorrect(colors)) {
  ...
  private makeYellowCross(): void {
    ...
    while (!this.hasYellowCross() && attempts < maxAttempts) {
      ...
      switch (pattern) {
        case 'dot':
          this.addMoves([Move.F, Move.R, Move.U, Move.R_PRIME, Move.U_PRIME, Move.F_PRIME], 'Dot to L-shape', stage)
          break
        case 'L':
          // Position L correctly
          this.positionLShape()
          this.addMoves([Move.F, Move.R, Move.U, Move.R_PRIME, Move.U_PRIME, Move.F_PRIME], 'L-shape to cross', stage)
          break
        case 'line':
          // Position line horizontally
          this.positionLine()
          this.addMoves([Move.F, Move.R, Move.U, Move.R_PRIME, Move.U_PRIME, Move.F_PRIME], 'Line to cross', stage)
          break
      }
  ...
  private hasYellowCross(): boolean {
    const state = this.cube.getState()
    return state.up.colors[0][1] === Color.YELLOW &&
  ...
  private positionYellowCross(): void {
    ...
    while (!this.isYellowCrossPositioned() && attempts < maxAttempts) {
      ...
      if (correctEdges === 0 || correctEdges === 1) {
        this.addMoves([Move.R, Move.U, Move.R_PRIME, Move.U, Move.R, Move.U2, Move.R_PRIME], 'Position edges', stage)
      } else if (correctEdges === 2) {
        // Position correct edges
        this.positionCorrectEdges()
        this.addMoves([Move.R, Move.U, Move.R_PRIME, Move.U, Move.R, Move.U2, Move.R_PRIME], 'Swap edges', stage)
      }
  ...
  private isYellowCrossPositioned(): boolean {
    const state = this.cube.getState()
    return state.front.colors[0][1] === Color.GREEN &&
           state.right.colors[0][1] === Color.RED &&
           state.back.colors[0][1] === Color.BLUE &&
           state.left.colors[0][1] === Color.ORANGE
  }
  ...
  private positionYellowCorners(): void {
    ...
    while (!this.areYellowCornersPositioned() && attempts < maxAttempts) {
      ...
      if (correctCorners.length === 0) {
        ...
        this.addMoves([Move.U, Move.R, Move.U_PRIME, Move.L_PRIME, Move.U, Move.R_PRIME, Move.U_PRIME, Move.L], 'Cycle corners', stage)
      } else {
  ...
  private areYellowCornersPositioned(): boolean {
    ...
    const corners = [
      { pos: [state.up.colors[2][2], state.front.colors[0][2], state.right.colors[0][0]], colors: [Color.GREEN, Color.RED] },
      { pos: [state.up.colors[0][2], state.right.colors[0][2], state.back.colors[0][0]], colors: [Color.RED, Color.BLUE] },
      { pos: [state.up.colors[0][0], state.back.colors[0][2], state.left.colors[0][0]], colors: [Color.BLUE, Color.ORANGE] },
      { pos: [state.up.colors[2][0], state.left.colors[0][2], state.front.colors[0][0]], colors: [Color.ORANGE, Color.GREEN] }
    ]
  ...
  private findCorrectlyPositionedCorner(): number[] {
    ...
    const corners = [
      { pos: [state.up.colors[2][2], state.front.colors[0][2], state.right.colors[0][0]], colors: [Color.GREEN, Color.RED], index: 0 },
      { pos: [state.up.colors[0][2], state.right.colors[0][2], state.back.colors[0][0]], colors: [Color.RED, Color.BLUE], index: 1 },
      { pos: [state.up.colors[0][0], state.back.colors[0][2], state.left.colors[0][0]], colors: [Color.BLUE, Color.ORANGE], index: 2 },
      { pos: [state.up.colors[2][0], state.left.colors[0][2], state.front.colors[0][0]], colors: [Color.ORANGE, Color.GREEN], index: 3 }
    ]
